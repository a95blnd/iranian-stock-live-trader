...
class DashboardScreen:
    ...
    async def change_value_summin(self, arg_id, best_buy_price, best_sell_price, signal):

        for for_key, nested_list in self.summin_data.items():
            if arg_id in nested_list:
                id = arg_id
                key = for_key
                index = nested_list.index(id)
                self.summin_value[key][index] = float(best_buy_price) if index == 1 else float(best_sell_price)
                self.summin_signal_object[key][index] = signal

                if not 0 in self.summin_value[key]:
                    result = round(self.summin_value[key][0] - (2 * self.summin_value[key][1]) + self.summin_value[key][2], 4)
                    if result != self.summin_value_check[key]:
                        # self.scrollable_text.add_item(
                        #     symbol=key,
                        #     f1=self.summin_value[key][0],
                        #     f2=self.summin_value[key][1],
                        #     f3=self.summin_value[key][2],
                        #     result=result
                        # )
                                    ...
                                    if 'instrumentId' in arguments:
                                        if signal.get('target') == 'MS':
                                            ...
                                            if depth_data:
                                                ...
                                                await self.change_value_summin(arg_id=instrument_id, best_sell_price=float(sells[0].get('price') or 0), best_buy_price=float(buys[0].get('price') or 0), signal=signal)
